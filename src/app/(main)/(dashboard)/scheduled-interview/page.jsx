...
import { getCandidates, getFeedback } from '@/app/actions/interview';
...
const page = async () => {
    ...
    const dataWithFeedback = await Promise.all(data.map(async (ele, idx) => {
        const feedback = await getFeedback(ele.id);
        const obj = ({ ...ele, feedbackArr: feedback })
        return obj;
    }));

    return (
        <div className="w-full container mx-auto py-10">
            <h1 className="text-xl sm:text-3xl font-bold">Interviews With Feedback</h1>
            <div className="flex flex-wrap mt-5">
                {
                    data.length === 0 ?
                        ...
                        :
                        dataWithFeedback.map((ele, idx) => <PreviousInterviewBox key={idx} data={ele} detailBox={true} />)
                }
            </div>
        </div>
    )
